...
RSpec.describe Player, type: :class do
  ...
  it 'has not won if it has cards' do
    player.hand << round.steal_card(value: 'H4')
    expect(player).not_to be_won
  end

  context 'has a meld' do
    let(:card_values_to_meld) { %w(H2 H3 H4) }
    let(:cards_to_meld) { round.find_cards_by_value(card_values_to_meld) }
    let(:meld) { round.find_meld(cards_to_meld) }

    before do
      %w(H2 H3 H4 D2 D3).each{|card| player.hand << round.steal_card(value: card)}
